 ...
 */
public class CommentsVendorRecyclerAdapter extends RecyclerView.Adapter<CommentsVendorRecyclerAdapter.ViewHolder> {

    public List<VendorComment> commentsList;

    public Context context;



    public CommentsVendorRecyclerAdapter(List<VendorComment> commentsList ){
    ...
    @Override
    public CommentsVendorRecyclerAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {

        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.comment_list_item, parent, false);
        context = parent.getContext();

        return new CommentsVendorRecyclerAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(final CommentsVendorRecyclerAdapter.ViewHolder holder, int position) {

        holder.setIsRecyclable(false);
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        String data = prefs.getString("blog_post_id", ""); //no id: default value
        String userName = commentsList.get(position).getCommentAuthor();

        if(commentsList.get(position).getCommentDate() != null) {
            String time = commentsList.get(position).getCommentDate();
        ...
        String commentMessage = commentsList.get(position).getCommentContent();
        holder.setComment_message(commentMessage);

        VendorComment c = commentsList.get(position);
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        private void fillComment(String userName, VendorComment comment, ExpandableTextView commentTextView, TextView dateTextView) {
            try {
                Spannable contentString = new SpannableStringBuilder(userName + "   " + comment.getCommentContent());
